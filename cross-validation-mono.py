...
from bin.dataset import SeqDataset_Mono, SeqDataset_BEND, SeqDataset_SE, SeqDataset_SE_BEND
...
for fold, (train_idx, val_idx) in enumerate(kf.split(np.arange(len(train_sigma70)))):
    ...
    train_seqs, train_labels = sliding(train_df, random_sampling=True)
    val_seqs, val_labels = sliding(val_df, random_sampling=True)
    
    # Create dataset
    train_set = SeqDataset_Mono(train_seqs, train_labels)
    val_set = SeqDataset_Mono(val_seqs, val_labels)
    ...
    for epoch in range(1000):
        ...
        if val_loss_his[-1] < 0.00000000000000001 + np.min(loss_val_history):
            patience = 0
            model = "best_model_fold_" + str(fold + 1) + ".pt"
...
unet = Unet1D(4, 1)
unet.cuda()
test_seqs, test_labels = sliding(ind_sigma70, ramdom_sampling=True)

test_set = SeqDataset_Mono(test_seqs, test_labels)
