...
for fold, (train_idx,val_idx) in enumerate(kf.split(np.arange(len(train_sigma70)))):
    ...
    # Create dataset
    train_set = SeqDataset_SE(train_seqs, train_labels, energy_ref_normed)
    val_set = SeqDataset_SE(val_seqs, val_labels, energy_ref_normed)
    ...
    for epoch in range(1000):
        ...
        if val_loss_his[-1] < 0.00000000000000001 + np.min(loss_val_history):
            patience = 0
            model = "best_model_SE_fold_" + str(fold + 1) + ".pt"
...
test_set = SeqDataset_SE(test_seqs, test_labels, energy_ref_normed)
